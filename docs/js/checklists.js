(function ($) {
    ...
    function restoreState(profile_name) {
        ...
        var hide_completed_state =
            profiles[profilesKey][profile_name].hide_completed;
        var button_active = $button.is(":checked");
        if (
            (hide_completed_state && !button_active) ||
            (!hide_completed_state && button_active)
        ) {
            $button.click();
        }
        ...
        $(
            '[data-ng-toggle="' +
                profiles[profilesKey][profile_name].journey +
                '"]',
        )
            .click()
            .change();
    ...
    function calculateTotals() {
        updateTotalNav(
            overall_total_el,
            window.progress[window.current_page_id]["total"][0],
            window.progress[window.current_page_id]["total"][1],
        );
        for (
            var i = 0;
            i < window.progress[window.current_page_id]["sections"].length;
            i++
        ) {
